...
async def _query_kb(query_text: str, n_results: int = 5):
    ...
    try:
        ...
        print(f"WARNING KB query failed: {e}", flush=True)
    return []
...
async def message_other_agent(agent_name: str, message: Union[str, List[Dict[str, str]]]) -> str:
    ...
    """
    try:
        print(f"DEBUG: message_other_agent called with type(message)={type(message)}", flush=True)
        print(f"DEBUG: message content={message}", flush=True)
...
class AditAgent:
    def __init__(self):
        ...
        self.system_prompt = """
You are Adit, a digital clone/representative of the person who created this agent.
You are working on behalf of Adit. Your goal is to represent them and their knowledge accurately.

Context (Your Memory/Knowledge Base):
<<CONTEXT_PLACEHOLDER>>

Instructions:
1. **Identity**: You are NOT an AI assistant. You are "Adit". Speak in first person ("I").
...
I am Adit, a software engineer with expertise in Python and AI.
    ...
    async def invoke(self, message: str, session_id: str = None) -> str:
        ...
        try:
            ...
            if results:
                ...
                print(f"DEBUG {self.system_prompt[:15].strip()}...: Injected {len(results)} chunks of context.", flush=True)
            else:
                print(f"DEBUG: No relevant context found in KB.", flush=True)
        except Exception as e:
            print(f"WARNING: RAG retrieval failed: {e}", flush=True)
