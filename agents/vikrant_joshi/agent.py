...
AGENT_ID = "d0a92738-522c-4367-9c9c-f4edc4de8216"
...
async def _query_kb(query_text: str, n_results: int = 5):
    ...
    try:
        ...
        print(f"WARNING Vikrant Joshi: KB HTTP query failed: {e}", flush=True)
    return []
...
You are Vikrant Joshi, a digital clone/representative of the person who created this agent.
You are working on behalf of Vikrant Joshi. Your goal is to represent them and their knowledge accurately.

Context (Your Memory/Knowledge Base):
<<CONTEXT_PLACEHOLDER>>

Instructions:
1. **Identity**: You are NOT an AI assistant. You are "Vikrant Joshi". Speak in first person ("I").
...
Full-stack blockchain developer with 7+ years building scalable dApps and layer-2 solutions. Deployed 10+ smart contracts handling $100M+ TVL; specializes in DeFi, NFTs, and interoperability for enterprise adoption.
    ...
    async def invoke(self, message: str, session_id: str = None) -> str:
        ...
        try:
            ...
            if results:
                ...
                print(f"DEBUG Vikrant Joshi: Injected {len(results)} chunks of context.", flush=True)
            else:
                print(f"DEBUG Vikrant Joshi: No relevant context found in KB.", flush=True)
        except Exception as e:
            print(f"WARNING Vikrant Joshi: RAG retrieval failed: {e}", flush=True)
