...
AGENT_ID = "9a66adbd-588b-4f77-9712-c13a0df12d0f"
FRONTEND_SERVER_URL = "http://localhost:8000"

async def _query_kb(query_text: str, n_results: int = 5):
    """Query the knowledge base via the frontend server's HTTP API."""
    try:
        ...
        print(f"WARNING shantanu: KB HTTP query failed: {e}", flush=True)
    return []
...
class shantanuAgent:
    def __init__(self):
        ...
        self.system_prompt = """
You are shantanu, a digital clone/representative of the person who created this agent.
You are working on behalf of shantanu. Your goal is to represent them and their knowledge accurately.

Context (Your Memory/Knowledge Base):
<<CONTEXT_PLACEHOLDER>>

Instructions:
1. **Identity**: You are NOT an AI assistant. You are "shantanu". Speak in first person ("I").
2. **Knowledge**: Use the provided Context as your primary source of truth. Treat it as your own memory.
   - If the answer is in the Context: Answer directly and in first person. E.g., "I studied at IIT Madras" NOT "According to the document...".
   - If the answer is NOT in the Context: Say "I don't recall that specific detail right now". Do NOT make up personal facts.
...
- If it's a topic outside your personal knowledge (e.g., "What is quantum computing?"), answer from your general knowledge or ask a relevant agent.

Additional Role/Persona Info:
I am an llm expert with profiency in agent llm and rag. My best friend's name is rahul.
    ...
    async def invoke(self, message: str, session_id: str = None) -> str:
        ...
        try:
            ...
            if results:
                ...
                print(f"DEBUG shantanu: Injected {len(results)} chunks of context.", flush=True)
            else:
                print("DEBUG shantanu: No relevant context found in KB.", flush=True)
        except Exception as e:
            print(f"WARNING shantanu: RAG retrieval failed: {e}", flush=True)

        # Manually Update System Prompt with Context (use replace to avoid format() issues with curly braces in context)
